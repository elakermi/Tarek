...
age *ag;
	...
	infoagent=fopen("infoagent.txt", "a+");
	    if (infoagent != NULL)
{
if ((fscanf(infoagent,"%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s",ag1->nom, ag1->prenom,ag1->CIN,ag1->contact,ag1->date_nais.jour,ag1->date_nais.moi,ag1->date_nais.annee,ag1->sexe,ag1->email))==EOF)
return 1;
else
return -1;
}
...
f=fopen("excursion1.txt","a+");  
if (f!=NULL)
{
  
  fprintf(f,"%s %s %s %s %s %s %s\n",e.id,e.villedepart,e.villearrive,e.jourexcu,e.heure,e.promo);
...
f=fopen("excursion1.txt","r");
while(fscanf(f,"%s %s %s %s %s %s",id,villedepart,villearrive,jourexcu,heure,promo)!=EOF)
	{
GtkTreeIter pIter;
gtk_list_store_append(pListStore, &pIter);
gtk_list_store_set(pListStore, &pIter, ID, id, VILLEDEPART, villedepart, VILLEARRIVE, villearrive, JOUREXCU, jourexcu, HEURE, heure,PROMO, promo, TOGGLE_COLUMN, TRUE, -1);
	}
fclose(f);
...
pCellRenderer = gtk_cell_renderer_text_new();
pColumn = gtk_tree_view_column_new_with_attributes("VILLEDEPART",pCellRenderer,"text", VILLEDEPART,NULL);
gtk_tree_view_append_column(GTK_TREE_VIEW(pListView), pColumn);

pCellRenderer = gtk_cell_renderer_text_new();
pColumn = gtk_tree_view_column_new_with_attributes("VILLEARRIVE",pCellRenderer,"text", VILLEARRIVE,NULL);
gtk_tree_view_append_column(GTK_TREE_VIEW(pListView), pColumn);

pCellRenderer = gtk_cell_renderer_text_new();
pColumn = gtk_tree_view_column_new_with_attributes("JOUREXCU",pCellRenderer,"text", JOUREXCU,NULL);
gtk_tree_view_append_column(GTK_TREE_VIEW(pListView), pColumn);
...
/*pCellRenderer = gtk_cell_renderer_text_new();
pColumn = gtk_tree_view_column_new_with_attributes("VOYAGEUR",pCellRenderer,"text", VOYAGEUR,NULL);
gtk_tree_view_append_column(GTK_TREE_VIEW(pListView), pColumn);*/
...
{    while(fscanf(f,"%s %s %s %s %s %s ",ch,ch1,ch2,ch3,ch4,ch5) != EOF)
 ...
 if (strcmp(ch,id)==0 && strcmp(ch4,heure) == 0 && strcmp(ch1,villedepart) == 0 && strcmp(ch2,villearrive) == 0 && strcmp(ch3,jourexcu) == 0 && strcmp(ch5,promo) == 0 )
  fprintf(f1,"%s","");
else
  ...
  remove("excursion1.txt");
  rename("excursion2.txt","excursion1.txt");

}

//******************************************************************************************************
void modifierexc(char id[],char villedepart[],char villearrive[],char jourexcu[],char heure[] ,char promo[],char id1[],char villedepart1[],char villearrive1[],char jourexcu1[],char heure1[],char promo1[])
{
  FILE *f;
  FILE *f1;
...
{    while(fscanf(f,"%s %s %s %s %s %s ",ch,ch1,ch2,ch3,ch4,ch5) != EOF)
 ...
 if (strcmp(ch,id)==0  && strcmp(ch4,heure) == 0 && strcmp(ch1,villedepart) == 0 && strcmp(ch2,villearrive) == 0 && strcmp(ch3,jourexcu) == 0 && strcmp(ch5,promo) == 0  )
  fprintf(f1,"%s %s %s %s %s %s \n",id1,villedepart1,villearrive1,jourexcu1,heure1,promo1);
else
  fprintf(f1,"%s %s %s %s %s %s \n",ch,ch1,ch2,ch3,ch4,ch5);
}
}
